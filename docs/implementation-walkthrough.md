...
| `internal/cli/doctor.go` (filled) | `shiptrace doctor` — checks home is writable, hook binary on `$PATH`, all 6 hooks installed (the lifecycle hooks plus `SessionEnd` for teardown), and runs a **latency probe** (10 synthetic invocations against a temp `SHIPTRACE_HOME`, reports p50/p99). Catches drift early. | `ccsettings`, `exec.LookPath` |
